...
**Theory:** With stride_length_s = 5, overlapping windows might be counted
multiple times.
...
**Likelihood:** ⭐⭐⭐⭐⭐ **VERY HIGH** - This is almost certainly the
issue!
...
**Theory:** The final window extends past the actual audio due to
chunk_length_s = 30.
...
**Likelihood:** ⭐⭐⭐⭐⭐ **VERY HIGH** - This explains the 1 minute
delay!
...
**Theory:** The last chunk might complete processing before all its words
are streamed.
...
If on_chunk_end fires at step 1, but words stream in step 2, progress =
100% too early.
...
**Theory:** We calculate `totalSeconds = audio.length / 16000`, but this
might not account for processing overhead.
...
**Theory:** After transcription reports 100%, diarization ("Identifying
speakers...") runs, but words from last chunk still streaming.
...
**The most likely issue:** `on_chunk_end` fires when chunk audio processing
completes, but tokens are still being decoded and streamed via
`callback_function`.
...
**Very likely secondary issue:** Our offset calculation might cause last
chunks to report positions beyond totalSeconds.
...
**Related to #1 and #3:** The last chunk processing might complete quickly
but then stream words for a long time.

---

### 4. ⭐⭐⭐ Last window processes beyond audio (Cause #2)

**Partially mitigated:** We cap with `Math.min()`, but the issue is WHEN
on_chunk_end fires.
...
**Problem:** We report progress based on `on_chunk_end`, but words keep
streaming.

**Solution:** Only report 100% when `on_finalize` fires (actual
completion).
