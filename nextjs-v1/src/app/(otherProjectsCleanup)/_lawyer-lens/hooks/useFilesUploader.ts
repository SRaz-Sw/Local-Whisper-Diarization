...
export const useFilesUploader = (
  ...
  >({});
  const [activeUploads, setActiveUploads] = useState<Set<string>>(
    new Set(),
  );
  const chunkUploadControllers = useRef<Record<string, AbortController>>(
    {},
  );
  const uploadOperations = useRef<
    Record<string, { pause: () => void; resume: () => void }>
  >({});
    ...
    if (resumable) {
      try {
        const savedStates = localStorage.getItem(
          STORAGE_KEY_UPLOAD_STATES,
        );
        if (savedStates) {
          const parsedStates = JSON.parse(savedStates);

          // Filter to only include states that have uploadId and aren't complete
          const validStates = Object.entries(parsedStates).reduce(
            (acc, [key, state]) => {
              ...
              if (
                ...
                acc[key] = {
                  ...
                  uploadDate: new Date(
                    uploadState.uploadDate || Date.now(),
                  ),
                  fileId: key,
                };
              }
            ...
            toast.info(
              `You have ${Object.keys(validStates).length} unfinished upload(s)`,
              {
                action: {
                  label: "Resume All",
                  onClick: () =>
                    resumeAllUploads(Object.keys(validStates)),
                },
              },
            );
          }
  ...
  useEffect(() => {
    if (resumable && Object.keys(uploadProgress).length > 0) {
      ...
      const statesToSave = Object.entries(uploadProgress).reduce(
        (acc, [key, state]) => {
          if (state.uploadId) {
            acc[key] = {
  ...
  const clearUploadProgressEntry = useCallback(
    (fileId: string) => {
      setUploadProgress((prev) => {
  ...
  const updateProgress = useCallback(
    (fileId: string, progressData: Partial<UploadProgress>) => {
      setUploadProgress((prev) => {
  ...
  const uploadFileWithChunks = useCallback(
    async (
      file: File,
      fileId: string,
      ...
      try {
        ...
        // Split file into chunks
        const { chunks, totalChunks } = splitFileIntoChunks(
          file,
          chunkSize,
        );
        ...
        updateProgress(fileId, {
          ...
          uploadId,
          progress: Math.round(
            (existingUploadedChunks.length / totalChunks) * 100,
          ),
        });

        // Determine which chunks need to be uploaded
        const chunksToUpload = chunks.filter(
          (_, index) => !existingUploadedChunks.includes(index),
        );
        const chunkIndexesToUpload = [...Array(totalChunks).keys()].filter(
          (index) => !existingUploadedChunks.includes(index),
        );
        ...
        const pause = () => {
          ...
          chunkIndexesToUpload.forEach((index) => {
            const controller =
              chunkUploadControllers.current[`${fileId}-${index}`];
            if (controller) {
              controller.abort();
              delete chunkUploadControllers.current[`${fileId}-${index}`];
            }
        ...
        const processQueue = async () => {
          ...
          while (
            activeUploads < concurrentUploads &&
            currentChunkIndex < chunkIndexesToUpload.length
          ) {
  ...
  const checkResumableUpload = useCallback(
    async (fileId: string, file: File) => {
      ...
      try {
        ...
        if (
          existingProgress?.uploadId &&
          existingProgress.status !== "complete"
        ) {
          return {
            uploadId: existingProgress.uploadId,
            completedChunks: Array.from(
              { length: existingProgress.chunksUploaded || 0 },
              (_, i) => i,
            ),
          };
        }
  ...
  const uploadFileStandard = useCallback(
      ...
      try {
        ...
        const unsubscribe = storageService.onProgress((event) => {
          updateProgress(fileId, {
            ...
            status:
              uploadProgress[fileId]?.status === "error"
                ? "error"
                : "uploading",
          });
        });
        ...
        const uploadPromises = files.map(async (file, index) => {
          ...
          try {
            ...
            if (file.size > chunkSize) {
              ...
              const resumableInfo = await checkResumableUpload(
                fileId,
                file,
              );
