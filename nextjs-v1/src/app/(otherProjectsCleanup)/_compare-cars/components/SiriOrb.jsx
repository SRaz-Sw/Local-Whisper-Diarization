...
function SiriOrb({
  ...
  const audioScale = 1 + (audioLevel / 255) * 0.3; // Scale from 1.0 to 1.3 based on audio
  const audioBlur = blurAmount * (1 + (audioLevel / 255) * 0.5); // Increase blur with audio
  const audioAnimationSpeed = Math.max(
    5,
