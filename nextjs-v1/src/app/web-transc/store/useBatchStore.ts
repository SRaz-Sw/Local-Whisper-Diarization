...
const extractAudioDuration = async (
  file: File,
...
const deserializeBatchFile = (serialized: any): BatchFile | null => {
  ...
  if (
    serialized.status === "processing" ||
    serialized.status === "queued"
  ) {
    serialized.status = "cancelled";
    serialized.progress = 0;
    serialized.error =
      "Session interrupted - file needs to be re-uploaded";
...
export const useBatchStore = create<BatchStore>()((set, get) => ({
  ...
  addFiles: (newFiles: File[]) => {
    ...
    if (totalCount > 50) {
      console.warn(
        `❌ Cannot add ${newFiles.length} files. Max batch size is 50. Current: ${currentFiles.length}`,
      );
      return;
    }
    ...
    batchFiles.forEach((batchFile, index) => {
      extractAudioDuration(newFiles[index])
        .then((duration) => {
          if (duration !== undefined) {
            console.log(
              `✅ Duration extracted for ${batchFile.fileName}: ${duration.toFixed(1)}s`,
            );
        ...
        .catch((error) => {
          console.warn(
            `Failed to extract duration for ${batchFile.fileName}:`,
            error,
          );
        });
    });
  ...
  removeFile: (fileId: string) => {
    set((state) => {
      ...
      if (file.status === "processing") {
        console.warn(
          "Cannot remove file that is currently processing. Cancel it first.",
        );
        return state;
      }
  ...
  retryFile: (fileId: string) => {
    set((state) => {
      ...
      if (file.retryCount >= 3) {
        console.warn(
          "Max retry attempts (3) reached for file:",
          file.fileName,
        );
        return state;
      }
  ...
  updateFileEstimatedTime: (
    ...
    set((state) => ({
      files: state.files.map((f) =>
        f.id === fileId
          ? { ...f, estimatedTimeRemaining: estimatedTime }
          : f,
      ),
    }));
  },
  ...
  setFileStatus: (
    ...
    set((state) => {
      ...
      const hasProcessing = newFiles.some(
        (f) => f.status === "processing",
      );
      const batchStatus =
        !hasQueued && !hasProcessing ? "completed" : state.batchStatus;
  ...
  removeAvailableWorker: (id: string) => {
    set((state) => ({
      availableWorkerIds: state.availableWorkerIds.filter(
        (wId) => wId !== id,
      ),
    }));
  },
